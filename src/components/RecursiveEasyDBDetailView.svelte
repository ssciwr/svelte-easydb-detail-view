...
{#if fields.length > 0}
  {#if firstField.kind === "field" }
    <FieldDispatch field={firstField} data={data} table={table}/>
  ...
  {:else if firstField.kind === "splitter" }
    {#if firstField.type === "panel-begin" }
      <Panel fields={fields.slice(0, findMatch("panel-begin", "panel-end"))} data={data} table={table}>
        <svelte:self fields={fields.slice(1, findMatch("panel-begin", "panel-end"))} data={data} table={table}/>
      </Panel>
      <svelte:self fields={fields.slice(findMatch("panel-begin", "panel-end") + 1)} data={data} table={table}/>
      ...
      <Tabs fields={fields} data={data} table={table}>
        {#each findTabs() as tab}
          <TabItem fields={tab} data={data} table={table}>
            <svelte:self fields={tab.slice(1)} data={data} table={table}/>
          </TabItem>
        {/each}
      </Tabs>
